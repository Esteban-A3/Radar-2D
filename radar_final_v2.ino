...
// ---------------- Configuracion de rango maximo ----------------
const long DISTANCIA_MAXIMA_CM = 100; // todo lo que este mas lejos se reporta como -1
// Timeout del pulseIn calculado para ~100cm:
// tiempo(us) = distancia(cm) * 2 / 0.034, mas un margen de seguridad
const unsigned long TIMEOUT_PULSEIN_US = 6000; // suficiente para ~100cm + margen
...
long leerDistanciaCM() {
  ...
  long duracion = pulseIn(PIN_ECHO, HIGH, TIMEOUT_PULSEIN_US);

  if (duracion == 0) {
    return -1; // fuera de rango, no hubo eco dentro del timeout
  }

  long distancia = duracion * 0.034 / 2; // cm

  if (distancia > DISTANCIA_MAXIMA_CM) {
    return -1; // mas lejos del rango que nos interesa
  }
